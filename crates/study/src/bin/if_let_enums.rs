...
Your task is to implement the function process_text_message that takes a reference to a Message enum.
For the Text variant, the function should return "Processed Text: <content>", replacing <content> with the actual string.
...
pub fn process_text_message(message: &Message) -> String {
    ...
    if let Message::Text(txt) = &message {
        return format!("Processed Text: {}", txt);
    }
